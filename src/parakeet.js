...
export class ParakeetModel {
  ...
  async transcribe(audio, sampleRate = 16000, opts = {}) {
    ...
    if (enc.dims.length === 3 && enc.dims[0] === 1 && enc.dims[1] === D && enc.dims[2] === Tenc) {
      ...
      // Optimized transpose with 8x unrolled sequential memory write loop
      // (outer loop t, inner loop d) to avoid JIT loop overhead in V8
      for (let t = 0; t < Tenc; t++) {
        const tOffset = t * D;
        let d = 0;
        for (; d <= D - 8; d += 8) {
          const encOffset = d * Tenc + t;
          transposed[tOffset + d] = encData[encOffset];
          transposed[tOffset + d + 1] = encData[encOffset + Tenc];
          transposed[tOffset + d + 2] = encData[encOffset + 2 * Tenc];
          transposed[tOffset + d + 3] = encData[encOffset + 3 * Tenc];
          transposed[tOffset + d + 4] = encData[encOffset + 4 * Tenc];
          transposed[tOffset + d + 5] = encData[encOffset + 5 * Tenc];
          transposed[tOffset + d + 6] = encData[encOffset + 6 * Tenc];
          transposed[tOffset + d + 7] = encData[encOffset + 7 * Tenc];
        }
        for (; d < D; d++) {
