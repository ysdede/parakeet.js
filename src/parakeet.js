...
export class ParakeetModel {
  ...
  async transcribe(audio, sampleRate = 16000, opts = {}) {
    ...
    if (enc.dims.length === 3 && enc.dims[0] === 1 && enc.dims[1] === D && enc.dims[2] === Tenc) {
      ...
      // Optimized transpose: JIT optimizations in V8 favor simple loops over
      // manual cache-blocking which introduces significant loop overhead.
      // We unroll the inner loop 8x and prioritize sequential writes to 'transposed'.
