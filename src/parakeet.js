...
export class ParakeetModel {
  ...
  async transcribe(audio, sampleRate = 16000, opts = {}) {
    ...
    for (let t = startFrame; t < Tenc;) {
      ...
      // Optimization: Reading values directly from the typed array in a pure
      // branching loop without caching to local variables avoids forced assignment
      // overhead in V8 and is faster.
      for (; i < tLen; i += 8) {
        if (tokenLogits[i] > maxLogit) { maxLogit = tokenLogits[i]; maxId = i; }
