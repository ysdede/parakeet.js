...
export class ParakeetModel {
  ...
  async transcribe(audio, sampleRate = 16000, opts = {}) {
    ...
    for (let t = startFrame; t < Tenc;) {
      ...
      // Optimization: For pure branch loops like argmax, direct array access is faster
      // than caching values to local variables, as it avoids forced assignment overhead
      // on every iteration in V8.
      for (; i < tLen; i += 8) {
        if (tokenLogits[i] > maxLogit) { maxLogit = tokenLogits[i]; maxId = i; }
