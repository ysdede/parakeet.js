...
## 2024-11-20 - LCS matrix fill loop unrolling
Learning: In the `LCSPTFAMerger` dynamic programming matrix fill path, an 8x unrolled inner loop with local variables tracking previous states provides a significant ~2.2x speedup by reducing iteration overhead and allowing better instruction-level parallelism in V8.
Action: Apply loop unrolling for heavy matrix fill or DP operations.
