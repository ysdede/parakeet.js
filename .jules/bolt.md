...
## 2024-11-20 - Softmax math.exp 8x unrolling with direct subtraction
Learning: When unrolling the `Math.exp` accumulation loop, calculating `val * scale - maxScaled` directly inside `Math.exp` instead of computing `(val - max) * scale` and assigning it to an intermediate variable yields a measurable performance improvement (~4%) in V8 hot loops due to reduced intermediate operations.
Action: Utilize 8x loop unrolling paired with direct subtraction of a pre-scaled constant for tight floating-point accumulation loops over TypedArrays.
